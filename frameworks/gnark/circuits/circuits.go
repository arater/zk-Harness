...
import (
	...
	"github.com/zkCollective/zk-Harness/frameworks/gnark/circuits/nfa_regex"
...
func init() {
	...
	BenchCircuits["nfa_regex"] = &defaultCircuit{}
}
...
func (d *defaultCircuit) Circuit(size int, name string, opts ...CircuitOption) frontend.Circuit {
	...
	case "nfa_regex":
		return &nfa_regex.NfaRegexCircuit{}
	default:
...
func (d *defaultCircuit) Witness(size int, curveID ecc.ID, name string, opts ...WitnessOption) witness.Witness {
	...
	case "nfa_regex":
		...
		nfaRegexCircuit := nfa_regex.NfaRegexCircuit{}
		...
		w, err := frontend.NewWitness(&nfaRegexCircuit, curveID.ScalarField())
		if err != nil {
			panic(err)
		}
		return w
